...
class HDF5Store(object):
    ...
    def multithreadedsearch(self,capvec):
        ...
        if min_dist<3 and self.min_val:
...
class Table(db.Model):
    ...
    date_created=db.Column(db.DateTime, default=datetime.istnow)
...
cap = cv2.VideoCapture(0) #webcam"http://192.168.0.112:8081"
        ...
        for (xmin,ymin,xmax,ymax) in faces:
            ...
            if xmax-xmin > 130: #discard small detected faces
                ...
                try:
                    ...
                    alltable = [x.title.split(" - ")[0] for x in list(Table.query.all())]
                    print(alltable)
                    if employee_name.split(".")[0] not in alltable and employee_name.split(".")[0] is not None:
                        print("Adding")
                        title = employee_name.split(".")[0]
                        desc = "..."
                ...
                if employee_name!=None:
                    display_img = cv2.imread(os.path.join('images',employee_name))
                    pivot_img_size = 112
